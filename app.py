...
def authenticate_user(email, password):
    ...
    if verify_password(stored_password, password):
        ...
        current_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        c.execute("UPDATE users SET last_login = %s WHERE email = %s", (current_date, email))
        conn.commit()
        conn.close()
        return True
...
def update_profile(email, name, job_title, company):
    ...
    c.execute(
        "UPDATE users SET name = %s, job_title = %s, company = %s WHERE email = %s",
...
def get_user_profile(email):
    ...
    c.execute(
        "SELECT email, name, job_title, company, date_joined, last_login FROM users WHERE email = %s",
        (email,)
    )
...
def save_ranking_history(email, job_title, description, results):
    ...
    c.execute(
        "INSERT INTO ranking_history (email, timestamp, job_title, description, results) VALUES (%s, %s, %s, %s, %s)",
        (
            email,
...
def get_user_history(email):
    ...
    query = "SELECT id, timestamp, job_title, description, results FROM ranking_history WHERE email = %s ORDER BY timestamp DESC"
...
def show_resume_optimizer_page():
    st.title("📄 Resume Optimizer")
    st.markdown("### Get feedback on your resume against a job description.")
    ...
    # --- Analysis Button ---
    if st.button("🔍 Analyze My Resume", disabled=not (resume_file and job_description)):
        with st.spinner("Analyzing your resume..."):
            try:
                ...
                if "Error extracting text" in resume_text:
                    st.error("❌ Error processing resume file.")
                    return

                # Create the prompt for Gemini
                prompt = f"""
                As a career coach, please provide a detailed analysis of the following resume based on the provided job description.
                Offer actionable suggestions on how to improve the resume.
    ...
    if st.button("🔍 Rank Resumes", disabled=not (uploaded_files and job_description)):
        with st.spinner("🔍 Processing resumes..."):
            ...
            if resumes:
                ...
                # Get Gemini scores for top 10 candidates
                gemini_scores = []
                justifications = []
                for index, row in results_df.head(10).iterrows():
                    resume_text = resumes[file_names.index(row["Resume Name"])]
                    prompt = f"""
                    As a human resources expert, please provide a score out of 100 and a brief justification for the following resume based on the provided job description.
                    ...
                    **Resume:**
                    ...
                    """
                    response = model.generate_content(prompt)
                    # a regex to extract the score from the response
                    import re
                    score_match = re.search(r"(\d+)/100", response.text)
                    if score_match:
                        gemini_scores.append(int(score_match.group(1)))
                    else:
                        gemini_scores.append(0)
                    justifications.append(response.text)

                results_df["Gemini Score"] = pd.Series(gemini_scores)
                results_df["Justification"] = pd.Series(justifications)

                st.dataframe(results_df.drop(columns=["Raw Score", "Justification"]), hide_index=True)

                # Display detailed results with expanders
                for index, row in results_df.head(10).iterrows():
                        with st.expander(f"{row['Rank']}. {row['Resume Name']}"):
                            st.markdown(f"**Match Score:** {row['Match Score']}")
                            st.markdown(f"**Gemini Score:** {row['Gemini Score']}/100")
                            st.markdown(f"**Justification:** {row['Justification']}")

                            if st.button("Generate Feedback", key=f"feedback_{index}"):
                                with st.spinner("Generating feedback..."):
                                    resume_text = resumes[file_names.index(row["Resume Name"])]
                                    prompt = f"""
                                    As a human resources expert, please generate a professional feedback report for the following candidate.
                                    Highlight their strengths and suggest areas for improvement based on their resume.
                ...
                fig = px.bar(chart_data, x="Raw Score", y="Resume Name", orientation='h', title="Top Candidates")
                st.plotly_chart(fig)
...
">
    ...
    if st.session_state["authenticated"]:
        ...
        if st.sidebar.button("📄 Resume Optimizer", use_container_width=True):
